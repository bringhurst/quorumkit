...
CLIENT_BIN=$(resolve_example_binary "$mydir" block block_client)
if [[ -z "$CLIENT_BIN" ]]; then
    echo "block: block_client not found. Build the examples from the repo root with: cmake --preset conan-release -DBUILD_EXAMPLES=ON && cmake --build --preset conan-release" >&2
    exit 1
fi
